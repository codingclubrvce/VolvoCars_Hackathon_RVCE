...
import 'api_service_add_sparepart.dart';
...
class _SpareState extends State<SpareForm> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      final apiService = ApiServiceAddSparePart();
      try {

        bool result = await apiService.addSparePart(assetData);
        print("posted:$result");
        if (result) {
          Navigator.of(context).pop(); // Close the form dialog
          final apiServiceAssets = ApiServiceSpareParts();
          apiServiceAssets.fetchSpareParts(); // Refresh the asset list
        } else {
          print('Add failed');
        }
      } catch (e) {
